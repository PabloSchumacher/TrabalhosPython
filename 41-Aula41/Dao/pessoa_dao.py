...
class PessoaDao:
    ...
    def insert(self, pessoa:PessoaModel):
        self.cursor.execute(f"INSERT INTO 01_MDG_PESSOA (NOME, SOBRENOME, IDADE) VALUES ('{pessoa.nome}','{pessoa.sobrenome}','{pessoa.idade}')")
    ...
    def update(self, pessoa:PessoaModel):
        self.cursor.execute(f"UPDATE 01_MDG_PESSOA SET NOME = '{pessoa.nome}', SOBRENOME = '{pessoa.sobrenome}', IDADE = {pessoa.idade} WHERE ID = {pessoa.id}")
        self.connection.commit()
        return pessoa.__dict__
